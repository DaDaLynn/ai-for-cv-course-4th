...
import torch
from my_Species_Network import *
...
class MyDataset():
    def __init__(self, root_dir, annotations_file, transform=None):
        ...
        if not os.path.isfile(self.annotations_file):
            print(self.annotations_file + 'does no exist')
...
def train_model(model, criterion, optimzier, schduler, num_epochs=50):
    Loss_list = {'train': [], 'val': []}
        ...
        for phase in ['train', 'val']:
            ...
            for idx,data in enumerate(data_loaders[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss = criterion(x_species, label_species)
...
model, Loss_list, Accuracy_list_species = train_model(network, criterion, optimizer, exp_lr_scheduler, num_epochs=50)
...
plt.plot(x, y6, color='b', linestyle='-', marker='.', linewidth=1, label='va')
plt.legend()
...
def visualize_model(model):
    ...
    with torch.no_grad():
        for _, data in enumerate(data_loaders['val']):
            ...
            plt.title('predicted species:{}\n ground-truth species:{}'.format(SPECIES[preds_species], SPECIES[label_species]))
            plt.show()
