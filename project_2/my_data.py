...
if __name__ == '__main__':
    ...
    for i in range(len(train_set)):
        ...
        # please complete your code under this blank
        img  = Image.fromarray(img.numpy()[0, :, :])
        landmarks = landmarks.numpy().flatten()
        draw = ImageDraw.Draw(img)
        for i in range(np.size(landmarks) // 2):
            draw.point((landmarks[2*i], landmarks[2*i+1]), 'rgb(255, 0, 0)')
        img.show()		
        '''
        key = cv2.waitKey()
        if key == 27:
